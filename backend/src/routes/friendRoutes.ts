...
export const friendRouter = new Hono()
    ...
    .get("/other-user/:otherUserId", async (c) => {
        ...
        const result = await FriendService.getOneUserWithPosts(
            userId,
            otherUserId
        );
    ...
    .post("/send", async (c) => {
        ...
        // Validate otherUserId using the schema
        const validation = sendFriendRequestSchema.safeParse(
            await c.req.json()
        );
        ...
        const status = result.friendRequest
    ...
    .post("/respond", async (c) => {
        ...
        let result;
        if (accept === true) {
            result = await FriendService.acceptFriendRequest(
                userId,
                otherUserId
            );
        } else if (accept === false) {
            result = await FriendService.rejectFriendRequest(
                userId,
                otherUserId
            );
        } else {
